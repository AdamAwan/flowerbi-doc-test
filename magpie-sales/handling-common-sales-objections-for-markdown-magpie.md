...
**Response:** Markdown Magpie is designed with a “won’t leak” principle as a core guarantee. All AI work is abstracted behind provider-neutral interfaces. You can run the entire stack locally with your own models (e.g., a local Ollama instance or an OpenAI-compatible gateway on your network) without ever sending data to an external API. The default deployment uses self-hosted Postgres, and no data leaves your infrastructure unless you explicitly configure a remote AI provider. Even then, the system logs and indexes remain on your infrastructure. The source code is fully open-source for audit. **Supporting evidence** – the pitch deck lists “won’t leak” as a core promise; the architecture document describes provider strategies that keep data local; and the AI jobs contract allows any CLI agent to run without cloud dependencies.
...
- Every question creates a job that the watcher processes. The watcher routes the question, retrieves context, and generates a cited answer using the AI provider. Most answers require a single provider call; the embedding models are inexpensive to run.
...
**Supporting evidence** – the AI jobs documentation describes the watcher and job lifecycle; the ingestion docs show that embeddings can be done with open-source models; the architecture mentions scheduler-based crunches that run on interval rather than per-query.
